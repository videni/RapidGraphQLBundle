...
class FormExtension implements FormExtensionInterface
{
     ...
     */
    public function __construct(ContainerInterface $container, array $types, array $typeExtensions, iterable $guessers)
    {
    ...
    public function getTypeExtensions($name)
    {
        ...
        if (isset($this->typeExtensions[$name])) {
            foreach ($this->typeExtensions[$name] as $serviceId => $extension) {
    ...
    public function getTypeGuesser()
    {
        if (!$this->guesserLoaded) {
            ...
            $guessers = [];
            foreach ($this->guessers as $serviceId => $guesser) {
                $guessers[] = $guesser;
            }
            if (\count($guessers) > 0) {
